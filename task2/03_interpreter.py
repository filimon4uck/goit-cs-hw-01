...
class Parser:
    ...
    def term(self):
        ...
        node = self.factor()
        while self.current_token.type in (TokenType.MUL, TokenType.DEV):
            token = self.current_token
            if token.type == TokenType.MUL:
                self.eat(TokenType.MUL)
            elif token.type == TokenType.DEV:
                self.eat(TokenType.DEV)
